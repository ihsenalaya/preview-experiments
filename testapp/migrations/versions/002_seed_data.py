...
Inserts exactly 5 products across 3 categories.
SEED_PRODUCT_COUNT = 5 is hardcoded in tests/e2e.py — keep in sync.
"""
...
def upgrade() -> None:
    ...
    conn.execute(sa.text("""
        ...
        SELECT p.name, p.description, c.id, p.price::numeric, p.stock::int, p.discount_pct::numeric
        FROM (VALUES
    ...
    conn.execute(sa.text("""
        ...
        SELECT p.id, r.author, r.rating::int, r.body
        FROM products p
        JOIN (VALUES
